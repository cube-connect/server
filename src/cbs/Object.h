...
class MyObject {
  ...
  public:
     ...
     */
    template <class T, typename... Args> T *CreateComponent(Args &&...params) {
    ...
    template <class T> void RemoveComponents() {
        ...
        for (auto &comp : m_Components) {
            if (dynamic_cast<T *>(comp.get()) != nullptr) {
                MarkToDestroy(
                    reinterpret_cast<__gnu_cxx::__normal_iterator<std::unique_ptr<Component> *,
    ...
    void RemoveComponent(std::uint8_t id) {
        ...
        auto comp = std::find_if(m_Components.begin(), m_Components.end(),
    ...
    template <class T> T *GetComponent(std::uint8_t id) {
        Components_t::iterator it = std::find_if(m_Components.begin(), m_Components.end(),
     ...
     */
    template <class T> void Connect(PropertyOut<T> &subject, PropertyIn<T> &observer) {
        //        assert(subject.Owner()->Object().ID() == m_ID && observer.Owner()->Object().ID() == m_ID);
        m_MessageManager.Connect(subject, observer);
    }

    template <class M, class O, void (O::*F)(M)> void Connect(MessageOut<M> &sender, MessageIn<M, O, F> &receiver) {
        //        assert(sender.Owner()->MyObject().ID() == m_ID && receiver.Owner()->MyObject().ID() == m_ID);
        m_MessageManager.Connect(sender, receiver);
    }

    template <class O, void (O::*F)(void)> void Connect(TriggerOut &sender, TriggerIn<O, F> &receiver) {
     ...
     */
    template <class T> void Disconnect(PropertyOut<T> &subject, PropertyIn<T> &observer) {
        //        assert(subject.Owner()->Object().ID() == m_ID && observer.Owner()->Object().ID() == m_ID);
        m_MessageManager.Disconnect(subject, observer);
    }

    template <class M, class O, void (O::*F)(M)> void Disconnect(MessageOut<M> &sender, MessageIn<M, O, F> &receiver) {
        //        assert(sender.Owner()->MyObject().ID() == m_ID && receiver.Owner()->MyObject().ID() == m_ID);
        m_MessageManager.Disconnect(sender, receiver);
    }

    template <class O, void (O::*F)(void)> void Disconnect(TriggerOut &sender, TriggerIn<O, F> &receiver) {
        //        assert(sender.Owner()->Object().ID() == m_ID && receiver.Owner()->MyObject().ID() == m_ID);
        m_MessageManager.Disconnect(sender, receiver);
    }
  ...
  private:
    ...
    Components_t::size_type m_ToUpdate;  // Number of components in the middle to be updated with additional components
                                         // not marked as to update till the ToInitialize sections
