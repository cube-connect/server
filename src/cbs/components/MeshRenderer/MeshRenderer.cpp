...
MeshRenderer::MeshRenderer(const std::string &path, ShaderProgram::Type type) : Drawable(type) { LoadModel(path); }

void MeshRenderer::MakeConnectors(MessageManager &message_manager) { message_manager.Make(this, ModelIn); }

void MeshRenderer::Initialize() { Object().Scene().RegisterDrawCall(this); }

void MeshRenderer::Destroy() { Object().Scene().UnregisterDrawCall(this); }
...
std::vector<Texture> MeshRenderer::LoadMaterialTextures(aiMaterial *material, aiTextureType type,
                                                        std::string typeName) {
    std::vector<Texture> textures;
