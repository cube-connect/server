...
void DrawManager::RegisterCamera(Camera *camera) { m_Camera = camera; }

Camera *DrawManager::MainCamera() const { return m_Camera; }

void DrawManager::Skybox(const std::string &right, const std::string &left, const std::string &top,
...
void DrawManager::Background(const glm::vec3 &background) { m_Background = background; }
...
void DrawManager::CallDrawsOverNetwork(ENetHost *server) const {
    ...
    DrawingSnapshot drawing_snapshot = {camera_pos_flattened_fixed_size, camera_view_fixed_size_flattened,
                                        camera_proj_fixed_size_flattened, local_to_world_matrices};
